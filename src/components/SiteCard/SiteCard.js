...
const SiteCard = props => {
  ...
  return (
    ...
    <ReactCardFlip isFlipped={flipped} flipSpeedBackToFront=".8" flipSpeedFrontToBack="1">
      {/* FRONT OF CARD */}
      <Card
        className={props.page === 'editor' ? `card-user ${colors}` : 'card-user'}
        style={props.page === 'editor' ? { backgroundColor: `${editorColor}` } : null}
      >
        ...
        <Card className="card-user">
          <CardBody className="card-body">
            ...
            {firstNames.length !== 0 && search && (
              <>
                <div className="additional-info">FIRST NAME(S) FOUND - {firstNames}</div>
                <br />
              </>
            )}
            ...
            {lastNames.length !== 0 && search && (
              <>
                <div className="additional-info">LAST NAME(S) FOUND - {lastNames}</div>
                <br />
              </>
            )}
            ...
            {dashboard && (
              <>
                <div className="additional-info">CLAIMED - {account.claimedAt.toUTCString()}</div>
                <br />
              </>
            )}
